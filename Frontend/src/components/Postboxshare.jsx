...
export default function Postboxshare({ postId }) {
  const [book, setbook] = useState(false);
  const [comment, setcomm] = useState(false);
  const { user, token } = useContext(AuthContext);
  const [liked, setliked] = useState(false);
 async function getliked() {
   const data=await axios.get(`http://localhost:3000/likes/isliked/${postId}/${user.userId}`)
   setliked(data.data.liked)
   console.log(data.data)
 
 }
  useEffect(()=>{getliked()},[postId])
  ...
  return (
    <div className="all-share">
      <div
        className="each-share"
        onClick={() => {
          setcomm(true);
        }}
      >
